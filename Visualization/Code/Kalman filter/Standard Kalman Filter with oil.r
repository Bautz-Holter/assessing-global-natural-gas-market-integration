# Standard Kalman Filter with oil — Visualisation
# Input: State paths, residuals, MLE parameters, integration tests
# Output: Beta/delta impact plots, residual diagnostics, integration tables
# Interpretation: Beta and Delta are impact coefficients
...
if (length(missing_state_cols) > 0) {
  stop("state_paths.csv is missing expected columns: ",
       ...
       "\nRe-run the augmented Kalman filter with oil script.")
}
...
safe_range_symmetric <- function(x, pad = 1.05, fallback = 1) {
  x <- x[is.finite(x)]
  if (length(x) == 0) return(c(-fallback, fallback))
  lim <- max(abs(range(x, na.rm = TRUE))) * pad
...
for (i in seq_len(nrow(pairs))) {
  ...
  p_d <- plot_state_panel(
    df, breaks,
    y = "Delta", lo = "Delta_lower", hi = "Delta_upper",
    colour = state_colors["delta"],
    ref_y = 0,
...
# 3. Pair-level delta plots

cat("Generating delta plots ...\n")
...
for (i in seq_len(nrow(pairs))) {
  ...
  ggsave(file.path(plots_dir, paste0("Delta_", dep, "_", ind, ".png")),
         p_delta, width = 7.2, height = 4.0, units = "in", dpi = 300, bg = "white")
}
...
delta_y_lims <- safe_range_symmetric(
  c(all_sp$Delta_lower, all_sp$Delta_upper),
  pad = 1.05,
  fallback = 1
)
...
p_mega_d <- ggplot(all_sp, aes(x = Date)) +
  ...
  geom_ribbon(aes(ymin = Delta_lower, ymax = Delta_upper),
              fill = state_colors["delta"], alpha = 0.22) +
  geom_line(aes(y = Delta), color = state_colors["delta"], linewidth = 0.45) +
  facet_wrap(~ Pair, ncol = 4, scales = "fixed") +
  coord_cartesian(ylim = delta_y_lims) +
  scale_x_date(date_breaks = "2 years", date_labels = "%Y",
               expand = c(0.01, 0.01)) +
  labs(x = NULL, y = expression(delta[t])) +
  theme_thesis() +
  theme(
    strip.text    = element_text(size = 8),
    axis.text.x   = element_text(size = 7),
...
if (!gt_available) {
  ...
  mle_cols <- intersect(
    ...
    c("Dependent", "Independent",
      ...
      "AR_Denom_Caution", "Corr_gas_oil_regressors",
  ...
  gt_mle <- mle_pars |>
    ...
    fmt_number(
      columns = intersect(
        mle_cols,
        c("log_V_obs", "log_Q_beta", "log_Q_delta",
          "Phi1", "Phi1_SE", "Phi2", "Phi2_SE",
    ...
    cols_label(
      ...
      Q_delta_effectively_zero = md("Q<sub>&delta;</sub> near zero"),
    ...
    tab_source_note(source_note =
      "H is the measurement variance. Q beta and Q delta are state-innovation variances for the time-varying gas and oil coefficients.") |>
    format_academic_table()
...
cat("== Standard Kalman Filter with oil visualisation Finished ==\n")
